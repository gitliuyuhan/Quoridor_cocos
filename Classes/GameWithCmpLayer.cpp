...
bool GameWithCmpLayer::init()
{
    ...
    setNameAndLevel((Text*)_nameAndLevel->getChildByName("name"), (Text*)_nameAndLevel->getChildByName("level"), "lyh", 1);
    
    _nameAndLevel1 = _bgSprite->getChildByName("nameAndLevel1");
    if(GameWithCmpLayer::p_flag){
        setNameAndLevel((Text*)_nameAndLevel1->getChildByName("name"), (Text*)_nameAndLevel1->getChildByName("level"), "玩家107", 9);
        log("sfafa");
    }
    else
        setNameAndLevel((Text*)_nameAndLevel1->getChildByName("name"), (Text*)_nameAndLevel1->getChildByName("level"), "电脑", 9);
...
{
    showResultLayer(1, "lyh", 1, 0, SpriteFrameCache::getInstance()->getSpriteFrameByName("local_robot_head_0.png")->getTexture());
}

void GameWithCmpLayer::giveUp()
{
    showResultLayer(2, "lyh", 1, -10, SpriteFrameCache::getInstance()->getSpriteFrameByName("local_robot_head_0.png")->getTexture());
}
...
void GameWithCmpLayer::cmpMove()
{
    ...
    if(GameLayer::_play_mode == 0)
    {
        std::pair<Vec2, int> move = _cmp_ai.getNextMove();
        log("ai:type=%d,x=%d,y=%d",move.second,move.first.x,move.first.y);
        type = move.second;
        x = move.first.x;
        y = move.first.y;
    }
    if(GameLayer::_play_mode == 1)
    {
        USER->opponentWalkChess(type,x,y);
    }

    int ai_player = Players::PLAYER_ONE;
    if(GameLayer::_player == 1)
    {
        ai_player = Players::ME;
    }
...
//    
    switch(type){
        case 2:
            gotoMove(ai_player, x, y);
            break;
        case 0:
            gotoWall(ai_player, 0, x, y);
            break;
        case 1:
            gotoWall(ai_player, 1, x, y);
            break;
    }
...
void GameWithCmpLayer::checkEnd(float a)
{
    ...
    if(_posVt[ME].y == CHESSNUM-1)
    {
        ...
        showResultLayer(0, "lyh", 1, +10, Sprite::createWithSpriteFrameName("local_robot_head_0.png")->getTexture());

    }
    else if(_posVt[PLAYER_ONE].y == 0)
    {
        showResultLayer(2, "lyh", 1, -10, Sprite::createWithSpriteFrameName("local_robot_head_0.png")->getTexture());
    }
}
...
void GameWithCmpLayer::changeState()
{
    switch(_state)
    {
        case GameState::BEGIN:
            if(GameLayer::_player == 0)
            {
                setState(GameState::PLAYER_MOVE);
            }
            else if(GameLayer::_player == 1)
            {
                setState(GameState::CMP_MOVE);
            }
            break;
