...
int GameLayer::_play_flag;
int GameLayer::_player;
int GameLayer::_play_mode;
...
bool GameLayer::init()
{
    ...
    //_whiteButton->addTouchEventListener(CC_CALLBACK_2(GameLayer::touchButton, this));
    ...
    _blackListen->onTouchBegan = [this](Touch* touch, Event* event){
        ...
        if(this->isPoint(x, y))
        {
            _playerSpriteVt[_play_flag]->setPosition(startX+UNIT*5*x, startY+UNIT*5*y);
        }
        return true;
    };
    ...
    _blackListen->onTouchEnded = [this](Touch* touch, Event* event){
        ...
        if(_posVt[_play_flag].x == x && _posVt[_play_flag].y == y)
        {
            log("show");
            auto moves = getMoves(_posVt[_play_flag]);
            for(auto move : moves)
            {
        ...
        else if(this->checkMove(_posVt[_play_flag], Vec2(x, y)))
        {
            this->gotoMove(_play_flag, x, y);
            log("ME:type=2,x=%d,y=%d",x,y);
            if(_play_mode)
              USER->walkChess(2,x,y);
            this->playerMoveEnd();
        }
        else
        {
            _playerSpriteVt[_play_flag]->setPosition(startX+UNIT*5*_posVt[_play_flag].x, startY+UNIT*5*_posVt[_play_flag].y);
        }
//        log("%d %d %d shortlength", (int)_posVt[0].x, (int)_posVt[0].y, this->getShortPath(_posVt[0], 0));
    ...
    _hbarListen->onTouchEnded = [this](Touch* touch, Event* event){
        ...
        _hbarSprite->setVisible(false);
        log("%d h %d    %d", x, y, _wallNumVt[_player]);
        
        if(_wallNumVt[_player] > 0 && this->checkWall(0, x, y))
        {
            this->gotoWall(_player, 0, x, y);
            if(_play_mode)
              USER->walkChess(0,x,y);
            log("hME:type=0,x=%d,y=%d",x,y);
            this->playerMoveEnd();
        }
    };
    ...
    _vbarListen->onTouchEnded = [this](Touch* touch, Event* event){
        ...
        _vbarSprite->setVisible(false);
        log("%d v %d   %d", x, y, _wallNumVt[_player]);
        
        if(_wallNumVt[_player] > 0 && this->checkWall(1, x, y))
        {
            this->gotoWall(_player, 1, x, y);
            log("vME:type=1,x=%d,y=%d",x,y);
            if(_play_mode)
              USER->walkChess(1,x,y);
            this->playerMoveEnd();
        }
    };
